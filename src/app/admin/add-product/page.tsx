  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({ title: "", category: "", price: "", mainDesc: "", origin: "", inStock: true });
      ...
      <div className="max-w-4xl bg-[#111111] border border-[#262626] rounded-3xl p-10 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-2xl border border-[#262626] overflow-hidden">
            <div className="bg-[#1A1A1A] px-5 py-3 border-b border-[#262626]">
              <p className="text-xs font-black uppercase tracking-widest text-zinc-500">Stock Status</p>
            </div>
            <div className="flex gap-0">
              <button
                type="button"
                onClick={() => setFormData({ ...formData, inStock: true })}
                className={`flex-1 flex flex-col items-center gap-2 py-5 transition-all duration-200 ${
                  formData.inStock
                    ? "bg-amber-400 text-black"
                    : "bg-[#1A1A1A] text-zinc-500 hover:bg-[#222]"
                }`}
              >
                <span className="text-2xl">✅</span>
                <span className="text-xs font-black uppercase tracking-widest">In Stock</span>
                <span className={`text-[10px] font-semibold ${formData.inStock ? "text-black/60" : "text-zinc-600"}`}>
              ...
              >
                <span className="text-2xl">🚫</span>
                <span className="text-xs font-black uppercase tracking-widest">Sold Out</span>
                <span className={`text-[10px] font-semibold ${!formData.inStock ? "text-red-400/60" : "text-zinc-600"}`}>
