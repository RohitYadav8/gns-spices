...
import {
  Mail, Lock, User, Eye, EyeOff, ArrowRight, ChevronRight,
  Package, Settings, MapPin, LogOut, CheckCircle2
} from "lucide-react";
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (!isLogin) { setActiveTab("Addresses"); setIsEditingProfile(true); }
      } else {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/user/profile", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: user._id, name: editName, phone: editPhone,
          addressLine: editAddress, landmark: editLandmark,
          city: editCity, postalCode: editPostalCode,
        }),
      });
      const data = await res.json();
      if (data.success) {
    ...
    return (
      <>
        ...
        <main className="relative min-h-screen overflow-hidden bg-black px-6 pt-32 pb-20 flex justify-center">
          ...
          <h1 className="pointer-events-none absolute left-1/2 top-5 -translate-x-1/2 select-none text-[140px] font-black uppercase tracking-widest text-white/2 md:text-[220px]">GNS</h1>
          ...
          <div className="relative z-10 w-full max-w-xl overflow-hidden rounded-[32px] border border-white/10 bg-zinc-950 backdrop-blur-xl shadow-[0_0_60px_rgba(245,158,11,0.15)] mt-20">
            <div className="p-6 sm:p-10 lg:p-12">
              ...
              {showForgotPassword ? (
                <>
                  <div className="mb-8">
                    <h2 className="text-4xl font-extrabold tracking-tight text-white mb-2">Forgot Password</h2>
                    <p className="text-zinc-400">Enter your email — we'll send a reset link.</p>
                  </div>
                  ...
                  {forgotSuccess ? (
                    <div className="flex flex-col items-center text-center gap-4 py-8">
                      ...
                      <p className="text-zinc-400 text-sm">Check your email inbox and follow the link to reset your password.</p>
                      <button
                        onClick={() => { setShowForgotPassword(false); setForgotSuccess(false); setForgotEmail(""); }}
                        className="mt-2 text-amber-400 font-bold text-sm hover:underline"
                      >
                        Back to Login
                      </button>
                    </div>
                    ...
                    <form onSubmit={handleForgotPassword} className="space-y-5">
                      ...
                      {forgotError && <p className="text-red-400 text-sm font-semibold">{forgotError}</p>}
                      ...
                      <button
                        ...
                        className="w-full text-center text-sm text-zinc-500 hover:text-amber-400 transition-colors"
                      >
                        Back to Login
                      </button>
                    </form>
                ...
                <>
                  ...
                  <div className="mb-8">
                    ...
                    <p className="mt-3 text-zinc-400">
                      {isLogin ? "Login to continue management and shopping" : "Create your premium culinary spice account"}
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleAuthSubmit} className="space-y-5">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <input type="text" placeholder="Enter your full name" value={name} onChange={(e) => setName(e.target.value)}
                            className="h-14 w-full rounded-xl border border-white/10 bg-black pl-12 pr-4 text-sm font-semibold text-white placeholder:text-zinc-500 outline-none focus:border-amber-400" />
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input type="email" placeholder="Enter your email" value={email} onChange={(e) => setEmail(e.target.value)}
                          className="h-14 w-full rounded-xl border border-white/10 bg-black pl-12 pr-4 text-sm font-semibold text-white outline-none focus:border-amber-400" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input type={showPassword ? "text" : "password"} placeholder="Enter password" value={password} onChange={(e) => setPassword(e.target.value)}
                          className="h-14 w-full rounded-xl border border-white/10 bg-black pl-12 pr-12 text-sm font-semibold text-white outline-none focus:border-amber-400" />
                    ...
                    <button type="submit" disabled={loading}
                      className="mt-2 flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-amber-400 hover:bg-amber-500 text-sm font-black tracking-wider text-white shadow-lg transition-all">
      ...
      <main className="min-h-screen bg-black pt-32 pb-20 px-4 md:px-8">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="col-span-1 space-y-4">
            <div className="bg-zinc-950/80 rounded-2xl border border-white/10 overflow-hidden">
              ...
              <button onClick={() => logout()} className="w-full flex items-center gap-3 p-5 text-white hover:bg-amber-400 hover:text-white transition-all">
                  ...
                  <form onSubmit={handleProfileUpdate} className="space-y-8 max-w-2xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} disabled={!isEditingProfile} required
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" value={editPhone} onChange={(e) => setEditPhone(e.target.value)} disabled={!isEditingProfile} placeholder="Add your mobile number" required
                        className="w-full h-12 bg-zinc-950/80 border border-white/10 rounded-lg px-4 text-white font-semibold focus:outline-none focus:border-[#F48F68] disabled:opacity-70" />
                    </div>
                    {isEditingProfile && (
                ...
                <div>
                  ...
                  <form onSubmit={handleProfileUpdate} className="space-y-6 max-w-2xl">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={editAddress} onChange={(e) => setEditAddress(e.target.value)} disabled={!isEditingProfile} placeholder="House No., Street, Area" required
                        className="w-full h-12 bg-zinc-950/80 border border-white/10 rounded-lg px-4 text-white font-semibold focus:outline-none focus:border-[#F48F68] disabled:opacity-70" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-white/60 uppercase tracking-widest">Landmark</label>
                      <input type="text" value={editLandmark} onChange={(e) => setEditLandmark(e.target.value)} disabled={!isEditingProfile} placeholder="Near hospital, park, etc." required
                        className="w-full h-12 bg-zinc-950/80 border border-white/10 rounded-lg px-4 text-white font-semibold focus:outline-none focus:border-[#F48F68] disabled:opacity-70" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-white/60 uppercase tracking-widest">City / District</label>
                        <input type="text" value={editCity} onChange={(e) => setEditCity(e.target.value)} disabled={!isEditingProfile} required
                          className="w-full h-12 bg-zinc-950/80 border border-white/10 rounded-lg px-4 text-white font-semibold focus:outline-none focus:border-[#F48F68] disabled:opacity-70" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-white/60 uppercase tracking-widest">Postal / ZIP Code</label>
                        <input type="text" value={editPostalCode} onChange={(e) => setEditPostalCode(e.target.value)} disabled={!isEditingProfile} required
                          className="w-full h-12 bg-zinc-950/80 border border-white/10 rounded-lg px-4 text-white font-semibold focus:outline-none focus:border-[#F48F68] disabled:opacity-70" />
                      </div>
                    </div>
                    {isEditingProfile && (
                      ...
                      {orders.map((order: any) => (
                        <div key={order._id} className="border border-white/10 bg-zinc-900/50 rounded-2xl p-6 hover:border-amber-400/30 transition-all">
                          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/10 pb-4 mb-4 gap-4">
                            ...
                            <div>
                              <span className={`px-4 py-1.5 rounded-full text-xs font-black tracking-wider uppercase ${order.status === 'Pending' ? 'bg-amber-500 text-white' : order.status === 'Delivered' ? 'bg-amber-400 text-amber-500' : 'bg-[#8BDFDD]/20 text-teal-800'}`}>
                                {order.status}
                              </span>
                            </div>
                          </div>
