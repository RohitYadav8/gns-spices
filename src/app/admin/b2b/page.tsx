        ...
        <div className="flex gap-4 items-start">
          ...
          <div>
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              <span className="px-2 py-0.5 rounded-md bg-white/5 text-zinc-400 text-xs border border-white/10">
                {item.businessType}
              </span>
              <span className="px-2 py-0.5 rounded-md bg-white/5 text-zinc-400 text-xs border border-white/10">
                {item.city}
        ...
        <div className="flex flex-col items-end gap-3 justify-between">
          ...
          {item.status === "Pending" && (
            <div className="flex gap-2">
              <button
                onClick={onApprove}
                className="px-4 py-2 rounded-xl bg-green-500 text-black text-sm font-bold hover:bg-green-400 transition-all"
              >
                Approve
              </button>
              <button
                onClick={onReject}
                className="px-4 py-2 rounded-xl bg-red-500/80 text-white text-sm font-bold hover:bg-red-500 transition-all border border-red-400/30"
              >
                Reject
              </button>
            </div>
    ...
    <div className="min-h-screen bg-black p-6 md:p-10">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-black text-white">
          B2B <span className="text-amber-400">Management</span>
        </h1>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="rounded-2xl border border-white/10 bg-zinc-900 p-5">
          ...
          <p className="text-3xl font-black text-amber-400">
            {[...applications, ...quotes].filter(i => i.status === "Pending").length}
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-zinc-900 p-5">
          <p className="text-xs uppercase tracking-widest text-zinc-500 mb-1">Approved</p>
          <p className="text-3xl font-black text-green-400">
            {[...applications, ...quotes].filter(i => i.status === "Approved").length}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 border-b border-white/10">
        ...
        >
          ...
          <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${
            activeTab === "applications" ? "bg-black/20" : "bg-white/10"
          }`}>
            {applications.length}
          </span>
        </button>
        ...
        >
          Quote Requests
          <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${
            activeTab === "quotes" ? "bg-black/20" : "bg-white/10"
          }`}>
            {quotes.length}
          </span>
        </button>
